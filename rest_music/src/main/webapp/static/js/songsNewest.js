...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const pageSize = 10; // 페이지당 불러올 노래 개수
    const loadMoreButton = document.getElementById('toggleButton');
    ...
    const loadMoreSongs = () => {
        isLoading = true;
        axios.get(`../song/api/songs?page=${currentPage}&pageSize=${pageSize}`)
            .then(response => {
    ...
    row.innerHTML = `
        ...
        <td class="song-info">
            ...
            <div>
                ...
                ${song.artistName && song.artistId ? 
                    `<a href="/artist/songs?artistId=${song.artistId.trim()}"
        ...
        <td>
            <a href="#" class="btn btn-primary btn-sm play-btn icon-button" 
                id="listenBtn"
        ...
        <td>
            <button type="button" class="btn btn-secondary btn-sm icon-button" id="addCPList" 
                data-id="${song.songId}">
    ...
    tbody.appendChild(row);
                    ...
                    // 더 이상 로드할 노래가 없으면 "더보기" 버튼을 비활성화합니다.
                    if (songs.length < pageSize) {
                        loadMoreButton.disabled = true;
                        loadMoreButton.textContent = '모든 곡을 불러왔습니다.';
                    }
                } else {
                    // 더 이상 로드할 노래가 없으면 "더보기" 버튼을 비활성화합니다.
                    loadMoreButton.disabled = true;
            ...
            .catch(error => {
                console.error('노래를 불러오는 중 오류가 발생했습니다:', error);
                isLoading = false;
            });
    };
    ...
    const observer = new IntersectionObserver(entries => {
        if (entries[0].isIntersecting && !isLoading) {
            loadMoreSongs();  // 페이지가 감지되면 다음 페이지 데이터 요청
        }
    }, {
        rootMargin: '0px',
        threshold: 1.0
